...
#seq_len 16, dim 768, depth 3, heads 12, dim_head 128, mlp_dim 3072, attention_window 8, 
class Longformer(nn.Module):
    ...
    def forward(self, x):
        ...
        b, n, _ = x.shape
        # b, n = x.shape  batch * 16 *768
        #n就是sequence length,其实是frame，因为现在frame就是sequence的长度，一张图对应一个特征向量
        cls_tokens = repeat(self.cls_token, '() n d -> b n d', b = b)
        x = torch.cat((cls_tokens, x), dim=1) # cat了cls到头部
        #x, batch * 17 * 768
        ...
        # Pooling
        #获得cls token,  batch*?  ,不知道sequence的维度多少，但确实是cls token
    ...
    def forward_new(self, x):
        """
           用于 TensorFormer 特征提取：[B, T, D] -> [B, T, D]
        """
        b, n, _ = x.shape

        # 不添加 cls_token，只添加位置编码（注意位数要和 n 匹配）
        x += self.pos_embedding[:, :n]
        x = self.dropout(x)

        # 构造全 1 的 attention mask
...
class LongformerSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if extra_attention_mask is not None:
            ...
            # # 保存attention weight 到npy文件
            # np.save('/data/chx/code/VTN-modify/new.npy', attn_weights.cpu())

            # #保存注意力权重矩阵
            # a = np.load('/data/chx/code/VTN-modify/result.npy')
            # b = np.load('/data/chx/code/VTN-modify/new.npy')
            # result = np.concatenate((a, b), axis=0)
            # np.save('/data/chx/code/VTN-modify/result.npy', result)
